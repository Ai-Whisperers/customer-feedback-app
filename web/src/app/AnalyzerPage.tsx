...
import { ExportResults } from '../features/export/ExportResults';
...
export const AnalyzerPage: React.FC = () => {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      const response = await analyzerApi.uploadFile(file);
  ...
  const pollStatus = async (id: string) => {
    const interval = setInterval(async () => {
      try {
        const status = await analyzerApi.getStatus(id);
        setProgress(status.progress);

        if (status.state === 'SUCCESS') {
          clearInterval(interval);
          const results = await analyzerApi.getResults(id);
          setResults(results);
  ...
  const handleExport = async (format: 'csv' | 'xlsx') => {
    try {
      await analyzerApi.exportResults(taskId, format);
    } catch (err) {
          ...
          {appState === 'processing' && (
            <div className="animate-fadeIn">
              <ProgressTracker
                ...
                progress={progress}
                status="processing"
                message="Analizando comentarios con inteligencia artificial..."
              />
            </div>
          )}
