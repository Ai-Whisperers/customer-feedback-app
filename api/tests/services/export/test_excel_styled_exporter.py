"""Tests for Excel Styled exporter."""
...
class TestExcelStyledExporter:
    """Test suite for ExcelStyledExporter class."""

    def test_initialization(self):
        """Test ExcelStyledExporter initializes with all builders."""
        exporter = ExcelStyledExporter()
        assert exporter.summary_builder is not None
        assert exporter.details_builder is not None
        assert exporter.emotions_builder is not None
        assert exporter.pain_points_builder is not None
    ...
    def test_export_all(self, sample_results):
        """Test exporting all data with all sheets."""
        exporter = ExcelStyledExporter()
        content, filename = exporter.export(
            sample_results,
            ExportInclude.ALL,
    ...
    def test_styled_sheets_have_styling(self, sample_results):
        """Test that styled sheets have formatting applied."""
        exporter = ExcelStyledExporter()
        ...
        # Check that summary sheet has some styling
        ws = wb["Resumen"]

        # Check if first cell has font styling (headers should be styled)
        cell = ws.cell(1, 1)
        assert cell.font is not None
        assert cell.fill is not None

    def test_includes_nps_chart(self, sample_results):
        """Test that summary sheet includes NPS chart."""
        exporter = ExcelStyledExporter()
        content, _ = exporter.export(
            sample_results,
            ExportInclude.SUMMARY,
